 ...
 * @param {boolean} occupancyStatus - The occupancy status (true = occupied, false = available).
...
export const createIoTDataModel = async (
  ...
  try {
    ...
    if (!parkingSpaceId || !sensorId || occupancyStatus === undefined || !updatedAt) {
 ...
 */
export const getLatestOccupancyStatusModel = async (
  ctx: QueryCtx,
  parkingSpaceId: Id<"parking_spaces">
): Promise<object | null> => {
  try {
    ...
    console.error("Failed to get latest occupancy status:", error);
